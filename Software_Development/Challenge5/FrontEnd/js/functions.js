...
// Make an HTTP GET request for the reports information.
function getReport(report){
    let request = "";
    switch (report){
        case "date":
            let start = $("#start").val().toString();
            let end = $("#end").val().toString();
            request = "report-dates/" + start + "/" + end;
            break;
        case "status":
            request = "report-status";
            break;
        case "client":
            request = "report-clients";
            break;
    }
    $.ajax({
        //url: "http://localhost:80/api/Reservation/" + request,
        url: "http://152.70.211.166:80/api/Reservation/" + request,
        type: "GET",
        dataType: "JSON",
        success: function(answer){
            console.log(answer);
            displayReport(report, answer);
            $("#start").val("");
            $("#end").val("");
        }
    })
...
// Function to display the GET results.
function displayReport(report, item){
    $("#reportResult").empty();
    let newTable = "<table class='info-table'>";
    switch (report){
        case "date":
            newTable += "<caption>Reporte de Reservas por Fecha</caption>";
            newTable += "<thead>";
            newTable += "<tr>";
            newTable += "<th>ID</th>";
            newTable += "<th>INICIO</th>";
            ...
            break;
        case "status":
            newTable += "<caption>Reporte de Estado de Reservas</caption>";
            newTable += "<thead>";
            newTable += "<tr>";
            newTable += "<th>ESTADO</th>";
            newTable += "<th>TOTAL DE RESERVAS</th>";
            newTable += "</tr>";
            newTable += "</thead>";
            break;
        case "client":
            newTable += "<caption>Top de Clientes</caption>";
            newTable += "<thead>";
            newTable += "<tr>";
            newTable += "<th>TOTAL DE RESERVAS</th>";
            newTable += "<th>INFORMACIÓN DEL CLIENTE</th>";
            newTable += "</tr>";
            newTable += "</thead>";
            break;
    }
    ...
    if (report == "status"){
        newTable += "<tr>";
        newTable += "<td>Completadas</td>";
        newTable += "<td>" + item.completed + "</td>";
        newTable += "</tr>";
        newTable += "<tr>";
        newTable += "<td>Canceladas</td>";
        newTable += "<td>" + item.cancelled + "</td>";
        newTable += "</tr>";
    }
    for(i = 0; i < item.length; ++i){
        switch (report){
            case "date":
                ...
                newTable += "<td>" + item[i].status + "</td>";
                newTable += "<td>" + JSON.stringify(item[i].skate) + "</td>";
                newTable += "<td>" + JSON.stringify(item[i].client) + "</td>";
                newTable += "<td>" + JSON.stringify(item[i].score) + "</td>";
                newTable += "</tr>";
                break;
            case "client":
                newTable += "<tr>";
                newTable += "<td>" + item[i].total + "</td>";
                newTable += "<td>" + JSON.stringify(item[i].client) + "</td>";
                newTable += "</tr>";
                break;
        }
